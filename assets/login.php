...
$currentIP = encrypt(getIP());
$new_token = rand(100000, 999999);

$encryptedEmail = encrypt($_POST['email']);

$checkEmail = $conn->query("SELECT * FROM `users` WHERE `email` IN ('$email','$encryptedEmail')");
if ($checkEmail->num_rows == 1) {
    ...
    if ($qCheckPassword->num_rows == 1) {
        ...
        $username = decrypt($UserRow['username']);
        $token = $UserRow['token'];
        $lastIP = decrypt($UserRow['ip']);
                ...
                <html>
                    ...
                    <body>
                        <div class="container">
                            ...
                            <h1>New IP</h1>
                            <p>We detected a login from a new IP address, was this you?</p><br>
                            <br>
                            <p>If this was you, you can ignore/delete this email. If this was NOT you, please change your password to secure your account.</p>
                        </div>
                    </body>
                </html>
                ';
